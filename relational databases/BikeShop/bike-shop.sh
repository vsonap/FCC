...
RETURN_MENU()
{
  ...
  if [[ -z $CUSTOMER_ID ]]
    ...
    CUSTOMER_RENTALS=$($PSQL "SELECT bike_id, type, size
                              FROM bikes INNER JOIN rentals USING(bike_id) INNER JOIN customers USING(customer_id)
    ...
    if [[ -z $CUSTOMER_RENTALS ]]
      ...
      then
        ...
        RENTAL_ID=$($PSQL "SELECT rental_id
                           FROM rentals INNER JOIN customers USING(customer_id)
